...
因为刚才那个 `alias` 的方法是**针对你个人电脑的“独门秘籍”**（因为它绑定了 `/Users/zyh/...` 这个绝对路径）。

如果别人（或者你换台电脑）照着原来的 README 做，克隆下来后直接运行 `pip install -e .`，他们大概率会遇到和你一样的 `command not found` 错误，因为他们的终端不知道去哪里找这个命令。

为了让 README 对**所有人**（包括未来的你）都通用且有效，我们需要把安装步骤写得更“标准”一点，或者把刚才那个“配置别名”的骚操作变成通用的教程。

这是为你准备的 **README 更新版（安装部分）**。

### 核心修改点

1. **区分了“开发者模式”和“懒人模式”**。
2. **增加了“配置环境变量”的极客技巧**（用 `$PWD` 自动获取当前路径，而不是写死 `/Users/zyh...`）。

---

### 请用这段 Markdown 替换原 README 中的 [📦 安装] 和 [🚀 极速上手] 部分：
